            ...
            <tr>
                ...
                <td style="width: 60%; text-align: center; vertical-align: middle;">
                    ...
                    <h2 style="margin: 0; text-transform: uppercase;">
                        {{ $submission->supplier->nama ?? 'NAMA SUPPLIER' }}
                    </h2>
                ...
                <td style="width: 20%; text-align: center; vertical-align: top;">
                    ...
                    @if(file_exists($logoPath))
                        <img src="{{ $logoPath }}" width="100" style="object-fit: contain;">
                    @endif
                </td>
            </tr>
        </table>
            ...
            <tr>
                <td width="55%" style="padding-right: 20px;">
                    <div class="info-box">
                        ...
                        <p><strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($submission->tanggal)->format('d F Y') }}
                        </p>
                        <p><strong>Dapur:</strong> {{ $submission->kitchen->nama ?? '-' }}</p>
                        <p><strong>Alamat:</strong> {{ $submission->kitchen->alamat ?? '-' }}</p>
                        @if($submission->supplier)
                        <p><strong>Supplier:</strong> {{ $submission->supplier->nama ?? '-' }} ({{
                            $submission->supplier->kode ?? '-' }})</p>
                        <p><strong>Kontak Supplier:</strong> {{ $submission->supplier->kontak ?? '-' }} - {{
        ...
        <table class="layout-table" style="margin-top: 10px; border-bottom: 1px solid #ccc; padding-bottom: 10px;">
            <tr>
                <td style="width: 35%; vertical-align: top;">
                    ...
                    <p style="margin: 0; text-transform: uppercase; font-size: 14px;">
                        {{ $submission->kitchen->nama ?? '-' }}
                    </p>
            ...
            <tr>
                <td style="width: 30%;">
                    <div style="font-size: 13px; line-height: 1.6;">
                        <p style="margin: 0; font-weight: bold;">REKENING PEMBAYARAN :</p>

                        @forelse($submission->supplier->bank_account as $bank)
                            <div style="margin-bottom: 10px;">
                                {{-- <p style="margin: 0;">{{ strtoupper($submission->supplier->nama) }}</p> --}}
                                <p style="margin: 0;">{{ strtoupper($bank->bank_name) }} {{ $bank->account_number }}</p>
                                <p style="margin: 0;">A.N.
                                    {{ strtoupper($bank->account_holder_name ?? $submission->supplier->nama) }}
                                </p>
                        ...
                        <div style="height: 100px; margin-bottom: 2px; text-align: center;">
                            @if ($submission->supplier && $submission->supplier->ttd)
                                ...
                                @if(file_exists($ttdPath))
                                    <img src="{{ $ttdPath }}" alt="TTD Supplier"
                                        style="max-height: 100px; object-fit: contain;">
                                @else
                                    {{-- Saya tambahkan path-nya di sini agar jika masih gagal, Anda bisa melihat letak
                                    error-nya --}}
